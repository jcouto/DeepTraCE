...
defaults = dict(trailmap = dict(models_folder = pjoin(deeptrace_path,'models')),
                elastix = dict(path = None,
                               ...
                               atlas = pjoin(deeptrace_path, 'registration','annotation_10_lsfm_collapse_crop_flip_newf.nrrd'),
...
def read_atlas(atlas_path = deeptrace_preferences['elastix']['atlas'],
               ontology_path = deeptrace_preferences['elastix']['ontology']):
    '''
    ...
    import pandas as pd
    import nrrd
    ontology = pd.read_csv(deeptrace_preferences['elastix']['ontology'])
